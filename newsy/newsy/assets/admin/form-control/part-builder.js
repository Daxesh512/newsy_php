ak.form.controlConstructor["part_builder"] = ak.form.Control.extend({
	...
	ready: function () {
		...
		this.builder_wrap = this.builder_container.find(".part-builder-wrapper");
	...
	initBuilder: function () {
		...
		if (!this.builder_inited) {
			...
			if (this.is_customizer) {
				var part_builder = control.container.find(".ak-part-builder");
				part_builder.prepend(
					'<div class="ak-part-builder-toogle-handler"><i class="fa fa-angle-down"></i></div>'
				);

				$(".ak-part-builder-toogle-handler").on("click", function (e) {
					e.preventDefault();
	...
	setCurrentPart: function (part) {
		if (this.is_customizer) {
			if (part == "builder_header_mobile" || part == "builder_drawer") {
	...
	getCurrentBuilderData: function (current_part) {
		...
		if (
			...
			_builder_data = newsy_part_builder_loc.part_defaults[current_part];
		}
	...
	switch_part: function () {
		...
		control.container.find(".part-switcher > div").on("click", function (e) {
			e.preventDefault();
	...
	init_part_html: function () {
		...
		_.each(control_data["order"], function (row_index) {
			control.builder_wrap.append(
				control.get_row_html(control_data[row_index], row_index)
			);
		});
	...
	update_column_items: function (element) {
		...
		var column = $(element).parents(".part-builder-column").data("column");
	...
	update_rows: function () {
		...
		control.builder_container.find(".part-builder-row").each(function () {
		...
		control.builder_container
			...
			.on("click", function (e) {
				...
				if (!column_el.hasClass(align)) {
					// update elements
					column_el.removeClass("left center right").addClass(align);

					// save setting
					var setting = current_part + "[" + row + "][" + column + "][align]";
		...
		control.builder_container
			...
			.on("click", function (e) {
				...
				if (!column_el.hasClass(layout)) {
					...
					// save setting
					var setting = current_part + "[" + row + "][" + column + "][layout]";
	...
	get_row_html: function (row, row_index) {
		...
		_.each(row, function (column, column_index) {
			if (
				...
				column_html += control.get_columns_html(column, column_index);
			}
		});
	...
	get_item_html: function (item) {
		...
		var item_name = newsy_part_builder_loc[current_part]["elements"][item];
	...
	setValue: function (value) {
		...
		if (typeof value["builder_header_desktop"] !== "undefined") {
			$.each(
				$("#ak-form-control-builder_header_desktop").find(".ak-form-control"),
				function () {
			...
			$.each(
				$("#header_desktop_style_group_start").find(".ak-form-control"),
				function () {
					var setting = $(this).data("control-id");
					if (ak.form.Controls[setting]) {
						ak.form.Controls[setting].setDefault();
		...
		if (typeof value["builder_footer"] !== "undefined") {
			$.each(
				$("#ak-form-control-builder_footer").find(".ak-form-control"),
				function () {
					var setting = $(this).data("control-id");
					if (ak.form.Controls[setting]) {
						ak.form.Controls[setting].setDefault();
