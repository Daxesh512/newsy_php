...
class SinglePost extends TemplateAbstract {
	...
	public function init( $post = null ) {
		...
		$this->href            = apply_filters( 'newsy_get_permalink', esc_url( get_permalink( $this->id ) ) );
	...
	public function hook() {
		...
		add_action( 'newsy_single_post_after', array( $this, 'get_related_posts' ), $is_infinity ? 60 : 50 );
	...
	public function get_article_attrs() {
		...
		$attr .= ' data-type="post" data-id="' . $this->id . '" data-url="' . $this->href . '" data-title="' . $this->title_attribute . '"';
	...
	public function get_featured_image( $image_size = 'newsy_750x0', $inline = false, $auto_wrap = true ) {
		...
		if ( 'video' === $this->post_format ) {
			...
			if ( '' !== $video_output ) {
				echo "<div class=\"ak-post-featured\"><div class=\"ak-featured-cover\">{$video_output}</div></div>";
				return;
			}
		}
		...
		if ( ! $inline ) {
			$post_featured_image_credit = $this->get_option( 'featured_image_credit', '' );
	...
	public function get_category() {
		...
		if ( function_exists( 'newsy_elements_get_post_categories' ) ) {
			$output .= newsy_elements_get_post_categories( $this->id, 5 );
		} else {
	...
	public function get_social_share_top() {
		...
		if ( empty( $share_sites ) || $this->get_option( 'social_share_top' ) === 'hide' || ! function_exists( 'newsy_get_share_buttons' ) ) {
			return;
		}
		...
		$share = newsy_get_share_buttons( $share_sites, $this->id, $share_style, $share_count_type, $share_show_count );

		$share = apply_filters( 'newsy_single_post_social_share_top', $share, $this->id );
	...
	public function get_social_share_bottom() {
		...
		if ( empty( $share_sites ) || $this->get_option( 'social_share_bottom' ) === 'hide' || ! function_exists( 'newsy_get_share_buttons' ) ) {
			return;
		}
		...
		$share = apply_filters( 'newsy_single_post_social_share_bottom', $share, $this->id );
	...
	public function get_tags() {
		...
		if ( ! empty( $newsy_post_tags ) ) {
			$tags_spot_text = newsy_get_translation( 'Tags:', 'newsy', 'tags' ); // the default text for tags - on CPT we can overwrite this via the panel
	...
	public function get_related_posts() {
		...
		if ( $result && count( $result ) > 0 ) {
			foreach ( $result as $tax ) {
				$taxs[] = $related_tax . ':' . $tax->term_slug;
			}
		}
		...
		$atts = array(
			...
			'post'                => '-' . $this->id, // exclade cuurent post
	...
	public function get_reaction_voting_box() {
		...
		$output .= newsy_reaction_get_voting_box( $this->id );
		$output .= '</div>';
		$output .= '</div>';
