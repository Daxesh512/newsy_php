...
if ( ! function_exists( 'ak_single_image' ) ) {
	/**
	* @param $id
	* @param $size
	*
	* @return string
	*/
	function ak_single_image( $id, $size ) {
		$image_size = wp_get_attachment_image_src( $id, $size );
		$image      = get_post( $id );
		$percentage = round( $image_size[2] / $image_size[1] * 100, 3 );

		$thumbnail  = '<div class="ak-featured-thumb lazy-thumb" style="padding-bottom:' . $percentage . '%">';
	...
	add_filter( 'ak_single_image', 'ak_single_image', 11, 2 );
}
