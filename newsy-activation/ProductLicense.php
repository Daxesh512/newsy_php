...
class ProductLicense {
			...
			if ( 'error' === $request['status'] ) {
				if ( 'register-purchase' === $action ) {
					...
					if ( isset( $request['access_code'] ) ) {
						$this->register_access_code_info( $args['item_id'], $request['access_code'] );
					}
				} 
				$response = $request;
	...
	public function fetch_data( $url, $args = array() ) {
		...
		if ( is_wp_error( $raw_response ) ) {
			...
			throw new \Exception( $error_message, $raw_response->get_error_code() );
		}
		...
		throw new \Exception( sprintf( 'Server cannot connect to %s. Please contact with Support ', $parse_url['host'] ), $response_code );
	}
